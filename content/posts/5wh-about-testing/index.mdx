...
With new people to automation testing, the common thinking is writing tests takes time and should be written after finishing
with the main code. Writing tests takes time to be correct, but only writing after the main principle is often wrong.

So when should we implement tests? As my experience with real work, it depends on the type of below tasks.

### 3.1 Refactoring

You better MUST write tests before refactoring, don't believe in yourself or anyone. To be honest, when I'm doing refactoring
I don't believe in myself as well. This isn't not only from my experience but also from famous coders like Martin Fowler, and Uncle Bob,
also mentioned in their books.

>I strongly advise you to learn and use refactoring tools in IDE like IntelliJ, and Eclipse.
They have awesome tools to make the work faster and, no doubt, with accuracy. Features such as Extract method, Rename,
Inline variable, Inline fields,... - most of the refactoring techniques.

### 3.2 Developing

Regarding feature implementation, you can write tests after finished things, but you can also write before (Test Driven Development).

### 3.3 Fixing bugs

The same with refactoring, you're suggested to cover related cases with tests, it's good to reproduce bugs by failed tests
so that when the bugs are fixed, you have the tests implemented and ensure the bug never goes back.
